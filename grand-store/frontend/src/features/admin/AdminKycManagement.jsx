...
export default function AdminKycManagement({ onNotify }) {
  ...
  const [previewDoc, setPreviewDoc] = useState(null); // { url, title, type, user, idNumber }
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/auction/admin/bidders').catch(err => ({ data: [] }));
      setBidders(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      console.error('Error fetching bidder KYC records:', err);
      if (onNotify) onNotify('Failed to fetch bidder KYC records', 'error');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const handleApproveBidder = async (bidderId, bidderName) => {
    try {
      await api.put(`/auction/admin/bidders/${bidderId}/approve`, {
        ...
        biddingLimit: 25000,
        notes: 'Approved via Bidder KYC Compliance Management'
      });
      if (onNotify) onNotify(`Bidder KYC approved for ${bidderName}`, 'success');
      fetchData();
    } catch (err) {
  ...
  const submitRejection = async () => {
    ...
    try {
      ...
      setRejectModal({ isOpen: false, id: null, targetName: '', reason: '' });
      fetchData();
    } catch (err) {
    ...
    <div className="space-y-8 pb-20 text-[var(--color-ivory)]">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-white/[0.08] pb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl md:text-3xl font-serif text-white tracking-wide">
                Auction Bidders KYC & Verification
              </h1>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={refreshing}
            className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 text-[var(--color-ivory)] text-xs font-mono rounded-xl border border-white/10 transition-all shadow-sm cursor-pointer"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-[#0e0e0e]/90 border border-amber-500/30 rounded-2xl p-5 shadow-lg relative overflow-hidden">
          ...
          <div className="text-3xl font-serif text-white mt-3 font-semibold">{pendingBidders}</div>
          <p className="text-[11px] text-[var(--color-ivory-muted)] mt-1">
            Awaiting identity & age verification
          </p>
        </div>

        <div className="bg-[#0e0e0e]/90 border border-emerald-500/20 rounded-2xl p-5 shadow-lg">
          <span className="text-xs font-mono uppercase tracking-wider text-emerald-400">Approved Bidders</span>
          <div className="text-3xl font-serif text-white mt-3 font-semibold">{approvedBidders}</div>
          <p className="text-[11px] text-[var(--color-ivory-muted)] mt-1">
            Certified eligible auction bidders
          </p>
        </div>
        ...
        <div className="bg-[#0e0e0e]/90 border border-rose-500/20 rounded-2xl p-5 shadow-lg">
          ...
          <div className="text-3xl font-serif text-white mt-3 font-semibold">{rejectedBidders}</div>
          <p className="text-[11px] text-[var(--color-ivory-muted)] mt-1">
        ...
        <div className="bg-[#0e0e0e]/90 border border-white/10 rounded-2xl p-5 shadow-lg">
          ...
          <div className="text-3xl font-serif text-white mt-3 font-semibold">{totalBidders}</div>
          <p className="text-[11px] text-[var(--color-ivory-muted)] mt-1">
            All registered bidder KYC profiles
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[#0a0a0a] p-3 rounded-2xl border border-white/[0.08]">
        {/* Section Tag */}
        <div className="flex items-center gap-2 bg-black/40 px-4 py-2 rounded-xl border border-white/5 text-xs font-medium tracking-wider uppercase text-amber-300">
          <User size={14} />
          <span>Registered Auction Bidders</span>
          {pendingBidders > 0 && (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-mono bg-amber-500 text-black font-bold ml-1">
              {pendingBidders} Pending
            </span>
          )}
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3 text-[var(--color-ivory-muted)] font-mono text-xs">
          ...
          <span>Loading bidder KYC submissions from registry...</span>
        </div>
                      ...
                      <td className="py-4 px-4">
                        {b.idDocumentUrl ? (
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-300 border border-amber-500/30 rounded-lg text-xs font-mono transition-all group cursor-pointer"
                          >
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {isPending && (
                            <>
                              <button
                                ...
                                className="px-3 py-1.5 bg-emerald-500/20 hover:bg-emerald-500/30 text-emerald-300 border border-emerald-500/40 rounded-lg text-xs font-mono font-bold transition-all flex items-center gap-1 cursor-pointer"
                              >
                                <Check size={12} /> Approve
                              </button>
                              <button
                                onClick={() => handleOpenRejectBidder(b)}
                                className="px-3 py-1.5 bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/40 rounded-lg text-xs font-mono font-bold transition-all flex items-center gap-1 cursor-pointer"
                              >
                                <X size={12} /> Reject
                              </button>
                            </>
                          ...
                          {isApproved && (
                            <button
                              ...
                              className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-[var(--color-ivory)] border border-white/10 rounded-lg text-xs font-mono transition-all flex items-center gap-1 cursor-pointer"
                            >
                          ...
                          {isRejected && (
                            <button
                              onClick={() => handleApproveBidder(b._id, b.name)}
                              className="px-3 py-1.5 bg-white/5 hover:bg-white/10 text-amber-300 border border-amber-500/20 rounded-lg text-xs font-mono transition-all cursor-pointer"
                            >
                              Re-approve
                            </button>
                          )}
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-black/40">
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="p-2 rounded-xl bg-rose-500/20 hover:bg-rose-500/30 text-rose-300 border border-rose-500/30 transition-colors ml-2 cursor-pointer"
                >
                  <X size={16} />
                </button>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-fadeIn">
          <div className="bg-[#121212] border border-rose-500/30 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <div className="flex items-center gap-3 text-rose-400 mb-4">
              ...
              <h3 className="text-lg font-serif text-white font-medium">Reject Bidder KYC Verification</h3>
            </div>
            <p className="text-xs text-[var(--color-ivory-muted)] mb-4">
              Specify the legal compliance reason why <strong className="text-white">{rejectModal.targetName}</strong>'s bidder KYC was rejected. This will be sent directly to the user.
            </p>

            <textarea
              value={rejectModal.reason}
              onChange={(e) => setRejectModal(prev => ({ ...prev, reason: e.target.value }))}
              placeholder="e.g., ID photo is unreadable/blurry, document has expired, or applicant does not meet legal requirements."
              rows={4}
            ...
            <div className="flex items-center justify-end gap-3">
              <button
                onClick={() => setRejectModal({ isOpen: false, id: null, targetName: '', reason: '' })}
                className="px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-xs font-mono rounded-xl transition-all cursor-pointer"
              >
                Cancel
              </button>
              <button
                onClick={submitRejection}
                className="px-4 py-2 bg-rose-500 hover:bg-rose-600 text-white text-xs font-mono font-bold rounded-xl transition-all shadow-lg cursor-pointer"
              >
                Confirm Rejection
              </button>
            </div>
          ...
          <div className="bg-[#121212] border border-amber-500/30 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button
                ...
                className="px-4 py-2 bg-gradient-to-r from-amber-500 to-yellow-600 hover:brightness-110 text-black font-bold text-xs font-mono rounded-xl transition-all shadow-lg cursor-pointer"
              >
                Save Ceiling
              </button>
            </div>
