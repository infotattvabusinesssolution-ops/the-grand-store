...
export default function AuctionHostLayout() {
  ...
  // Allow admin OR auction hosts
  if (user.role !== 'auction_host' && user.role !== 'admin' && user.role !== 'super_admin') {
    return <Navigate to="/login" replace />;
  }
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <div className="hidden md:block text-sm tracking-widest text-[#c9a35b] font-medium uppercase drop-shadow-[0_0_8px_rgba(212,175,55,0.5)]">
              {['admin', 'super_admin'].includes(user?.role) ? 'Admin Gateway' : 'Auction Manager Portal'}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            <div className="text-right hidden md:block">
              ...
              <div className="text-xs text-[#c9a35b] tracking-widest uppercase">{['admin', 'super_admin'].includes(user?.role) ? 'System Administrator' : 'Auction Organizer'}</div>
            </div>
            <div className="w-10 h-10 rounded-full bg-[#c9a35b] p-[1px]">
              <div className="w-full h-full bg-[#0a0a0a] rounded-full flex items-center justify-center">
                <Gavel size={18} className="text-[#c9a35b]" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside className={`w-64 bg-[#0a0a0a]/95 backdrop-blur-xl border-r border-white/[0.02] flex flex-col fixed top-0 bottom-0 left-0 z-50 overflow-y-auto transition-transform duration-300 ease-in-out ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
          <nav className="flex flex-col flex-1 p-6 gap-2 mt-2">
            ...
            <button onClick={() => handleNavigate('/auction-manager/dashboard')} className={navItemClass('/auction-manager/dashboard')}>
              <LayoutDashboard size={16} /> Dashboard
            </button>
            <button onClick={() => handleNavigate('/auction-manager/wallet')} className={navItemClass('/auction-manager/wallet')}>
              <Wallet size={16} /> Wallet & Payouts
            </button>

            <div className="text-[10px] text-[var(--color-ivory-muted)] uppercase tracking-widest mb-2 mt-4 pl-2">Auction Inventory</div>
            <button onClick={() => handleNavigate('/auction-manager/inventory')} className={navItemClass('/auction-manager/inventory')}>
              <Package size={16} /> Hosted Lots
            </button>
            <button onClick={() => handleNavigate('/auction-manager/auction-submit')} className={navItemClass('/auction-manager/auction-submit')}>
