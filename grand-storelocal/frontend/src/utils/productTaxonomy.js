...
export const isSouthAfricanProduct = (product = {}) => {
  if (!product) return false;

  // 1. Direct country property
  const country = cleanText(product.country || "").toLowerCase();
  if (country === "south africa" || country === "za") return true;

  // 2. Origin string
  const origin = cleanText(product.origin || "").toLowerCase();
  if (origin.includes("south africa") || origin.includes("za")) return true;

  // 3. Recognized South African wine and spirit regions & terroirs
  const saRegions = [
    "stellenbosch", "franschhoek", "paarl", "swartland", "robertson",
    "constantia", "walker bay", "elgin", "hemel-en-aarde", "durbanville",
    "cape town", "western cape", "cape winelands", "breede river",
    "tulbagh", "wellington", "klein karoo", "olifants river", "cape south coast",
    "calitzdorp", "cederberg", "overberg", "bot river", "darling", "still bay"
  ];
  const region = cleanText(product.region || "").toLowerCase();
  if (saRegions.some(reg => origin.includes(reg) || region.includes(reg))) {
    return true;
  }

  // 4. Recognized South African brands (checking brand, storeName, name, title)
  const saBrands = [
    "bain's", "bains", "three ships", "kwv", "klipdrift", "van ryn", "richelieu",
    "inverroche", "tshireletso", "queen mother", "inzalo", "leonista",
    "kanonkop", "meerlust", "hamilton russell", "boekenhoutskloof",
    "rust en vrede", "klein constantia", "springfield", "mullineux",
    "sadie family", "bouchard finlayson", "simonsig", "graham beck",
    "cap classique", "savanna", "castle lager", "black label", "windhoek"
  ];
  const brand = cleanText(product.brand || product.storeName || "").toLowerCase();
  const name = cleanText(product.name || product.title || product.productName || product.displayName || "").toLowerCase();
  
  if (saBrands.some(b => brand.includes(b) || name.includes(b))) {
    return true;
  }

  // 5. Tags or description checking
  const tagsStr = Array.isArray(product.tags) ? product.tags.join(" ").toLowerCase() : cleanText(product.tags || "").toLowerCase();
  if (tagsStr.includes("south africa") || tagsStr.includes("south african")) return true;

  const desc = cleanText(product.description || "").toLowerCase();
  if (desc.includes("south africa") || desc.includes("south african") || desc.includes("cape winelands")) {
    return true;
  }

  // 6. Name containing explicit South Africa or SA wine regions
  if (name.includes("south africa") || name.includes("south african") || saRegions.some(reg => name.includes(reg))) {
    return true;
  }
