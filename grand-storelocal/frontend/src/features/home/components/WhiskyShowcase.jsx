...
export default function WhiskyShowcase({ onAdd, onWish, onCompare, compareItems }) {
  ...
  const categoryData = categories?.find(c => c.name.toLowerCase() === 'whisky' || c.name.toLowerCase() === 'scotch');
  ...
  return (
    <>
      <section className="arrivals-section showcase-section relative w-full bg-[#0a0c0e] text-white pt-4 pb-6 md:pt-6 md:pb-8 overflow-hidden select-none" id="whisky" ref={sectionRef}>
        {/* Sacred African Sun / Chakra Background Engraving */}
        <ArrivalsMandala gradientId="mandala-whisky" position="top-right" />
            ...
            <div className="max-w-xl">
              <div className="inline-flex items-center gap-2 mb-2">
                <span className="text-[11px] font-semibold tracking-[0.22em] uppercase text-[#caa458]">
                  STORIED DISTILLERIES & RARE CASKS
                </span>
            ...
            >
              <div className="flex w-max animate-[marquee_20s_linear_infinite] sm:animate-[marquee_24s_linear_infinite] group-hover:[animation-play-state:paused] will-change-transform items-center">
                {marqueeBrands.map((brand, index) => (
                  <Link
                    to={`/shop?category=Whisky&brand=${encodeURIComponent(brand.name)}`}
