...
export default function AuctionHostLayout() {
  ...
  if (user.role !== 'auction_host' && user.role !== 'admin') {
    return <Navigate to="/login" replace />;
  }
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <div className="hidden md:block text-sm tracking-widest text-[#c9a35b] font-medium uppercase drop-shadow-[0_0_8px_rgba(212,175,55,0.5)]">
              {user?.role === 'admin' ? 'Admin Gateway' : 'Auction Manager Portal'}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-3">
            <div className="text-right hidden md:block">
              ...
              <div className="text-xs text-[#c9a35b] tracking-widest uppercase">{user?.role === 'admin' ? 'System Administrator' : 'Auction Organizer'}</div>
            </div>
